...
impl TokenData {
    /// 生成token
    pub fn create_token(&self) -> Result<String, MineError> {
        return match jsonwebtoken::encode(
            ...
            Ok(t) => Ok(t),
            Err(_) => Err(MineError::JwtError("JWTToken encode fail!")),
        };
    }

    /// 验证token是否有效
    pub fn verify(token: &str) -> Result<TokenData, MineError> {
        return match jsonwebtoken::decode::<TokenData>(
            ...
            Err(err) => match *err.kind() {
                ErrorKind::InvalidToken => return Err(MineError::JwtError("InvalidToken")),
                ErrorKind::InvalidIssuer => return Err(MineError::JwtError("InvalidIssuer")),
                _ => {
                    log::error!("InvalidToken other errors: {err}");
                    return Err(MineError::JwtError("InvalidToken other errors"));
                }
            },
        };
    }
